...
export const allUserWishList = asyncHandler(async(req, res, next) => {
    const wishList = await userModel.findById(req.user.id).select("wishList");

if (! wishList) {
    return next ( new Error ('you dont have any product yet' , { cause: 400 }))
}   
